...
mesh_file_path = "C:\\Users\\michi\\Documents\\WORK\\TUrobo\\scripts\\filesrhinotocompas\\from_lennart3.json" # change accordingly
...
for block in assembly.blocks():
    ...
    if node_text == goal_block:
        m_viewer.scene.add(block, show_faces=True, color=Color(0, 0.1, 0.9))
    else:
        m_viewer.scene.add(block, show_faces=False)
...
for block in assembly.blocks():
    ...
    if node_text == goal_block:
        viewer.scene.add(block, show_faces=True, color=Color(0, 0.1, 0.9))
    else:
